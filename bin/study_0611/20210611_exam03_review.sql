...
select ename as "사원 이름", nvl(mgr, 0) as "상관 사번" -- 타입에 맞는 기본값을 넣어야한다.
from emp
order by mgr -- mgr : 숫자타입 
;
...
select max(sal) - min(sal) as "급여 최고최저 차액"
from emp
;
...
group by job -- 직급별로 그룹핑
having min(sal) > 2000
order by min(sal) desc -- 급여 내림차순
;


--29. 각 부서에 대해 부서번호, 사원 수, 부서 내의 모든 사원의 평균 급여를 출력하시오.
--평균 급여는 소수점 둘째 자리로 반올림 하시오.
select deptno as "부서 번호",  count(*) as "부서별 사원 수", round(avg(sal), 2) as "부서별 평균 급여"
from emp
group by deptno
;
